---
theme: jekyll-theme-minimal
title: AI &amp; NLP Lab Software
author: Damir Cavar
permalink: /software/
layout: redirect
redirect_to: https://ai-science-labs.org/software/
---

# Software

## Quantum Tools

- [Natural Language Qu Kit](https://nlqk.ai/) ([NLQK](https://nlqk.ai/)) 


## Corpus Tools

- [The Hoosier Ellipsis Corpus](https://github.com/dcavar/hoosierellipsiscorpus) — see the subfolders for experiments using Logistic Regression, Large Language Models (GPT-4, Claude 3, Llama 3, …)


## JSON-NLP and NLP-Pipelines

- [JSON-NLP Schema](https://github.com/dcavar/JSON-NLP)
- NLP RESTful Microservice Architecture (coming soon)
- [Python JSON-NLP module](https://pypi.org/project/pyjsonnlp/)
- [Flair JSON-NLP module](/flairjsonnlp/)
- [NLTK JSON-NLP module](/nltkjsonnlp/)
- [Polyglot JSON-NLP module](/polyglotjsonnlp/)
- [spaCy JSON-NLP module](/spacyjsonnlp/)
- [Xrenner JSON-NLP module](/xrennerjsonnlp/)
- Stanford CoreNLP JSON-NLP module (coming soon)
- LingPipe JSON-NLP module (coming soon)
- OpenNLP JSON-NLP module (coming soon)
- MaltParser JSON-NLP module (coming soon)
- YAGO JSON-LD module (coming soon)
- DBpedia JSON-LD module (coming soon)
- ConceptNet JSON-LD module (coming soon)


## TIE-ML Tools

- [TIE-ML GitHub repo](https://github.com/dcavar/tieml)
- TIE-ML to TimeML (coming soon)
- TIE-ML GUI (coming soon)


## Rust Tools and Resources

- [AI &amp; NLP Lab Rust info and links](/rust/)

